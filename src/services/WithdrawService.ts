...
export default class WithdrawService {
    ...
    static async ConfirmWithdraw(data: { user: AuthUser, amount: number, bankId: number, action: string, pin: number }): Promise<CreateWithdrawRequestResponse> {
        try {
    ...
    private static validateWithdrawInput({ user, amount, bankId, action, pin, config }: {
        user: AuthUser, amount: number, bankId: number, action: string, pin: number, config: {
            error: boolean;
            message: string;
