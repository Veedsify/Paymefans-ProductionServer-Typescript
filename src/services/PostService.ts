...
export default class PostService {
      ...
      private static async processUserMediaSignedUrls(
            ...
            const processedMedia = await GenerateBatchSignedUrls(
                  userMedia.map((media) => ({
            ...
            return userMedia.map((media, index) => ({
                  ...media,
                  url: processedMedia[index]?.url || media.url,
                  poster: processedMedia[index]?.poster || media.poster,
                  blur: processedMedia[index]?.blur || media.blur,
            }));
      }
      ...
      private static async processMediaFileSignedUrls(
            ...
            return {
                  ...mediaFile,
                  url: signedMedia[0]?.url || mediaFile.url,
                  poster: signedMedia[0]?.poster || mediaFile.poster,
                  blur: signedMedia[0]?.blur || mediaFile.blur,
                  isSubscribed: true,
            };
      }
      ...
      static async CreatePost(
            ...
            try {
                  ...
                  if (removedMedia) {
                        const removeMedia =
                              await RemoveCloudflareMedia(removedMedia);
                  ...
                  const formattedContent = ParseContentToHtml(
                        content,
                        mentions || [],
                  );
                  const isWaterMarkEnabled =
                        WatermarkService.isUserWatermarkEnabled(user.id);
                  ...
                  if (mentions && mentions.length > 0) {
                        const validMentions =
                              await MentionService.validateMentions(
                                    mentions,
                                    authUserId,
                              );
                  ...
                  const resolvedPosts = await Promise.all(
                        posts.map(async (post) => {
                              ...
                              return {
                                    ...post,
                                    UserMedia:
                                          await this.processUserMediaSignedUrls(
                                                post.UserMedia,
      ...
      static async MyReposts({
            ...
            try {
                  ...
                  // Batch likes and reposts
                  const [likeData, repostsDb, purchasedPosts] =
                        await Promise.all([
                              RedisPostService.getMultiplePostsLikeData(
                                    postIdStrings,
                                    userId,
                  ...
                  const postRepostSet = new Set(
                        repostsDb.map((r) => r.post_id),
                  );
                  const purchasedPostsSet = new Set(
                        purchasedPosts.map((r) => r.post_id),
                  );
                  ...
                  const mediaChecked = await Promise.all(
                        media.map(async (mediaFile) => {
                              const processedMedia =
                                    await this.processMediaFileSignedUrls(
                                          mediaFile,
                                    );
                              return {
                                    ...processedMedia,
      ...
      static async GetOtherMedia({
            ...
            try {
                  ...
                  const [authUser, isSubscribed, media, payedPosts] =
                        await Promise.all([
                              query.user.findUnique({
                  ...
                  const resolvedMedia = await Promise.all(
                        media.map(async (mediaFile) => {
                              ...
                              return {
                                    ...
                                    hasPaid:
                                          hasViewPaidMediaFlag ||
                                          purchasedPostsSet.has(
                                                mediaFile.post_id,
                                          ) ||
                                          mediaFile.accessible_to !== "price",
                                    isSubscribed:
                                          hasViewPaidMediaFlag ||
                                          mediaFile.post.user.id ===
                                                Number(authUserId) ||
                                          !!isSubscribed,
                              };
                        }),
      ...
      static async GetUserPostByID({
            ...
            try {
                  ...
                  // Batch queries
                  const [subs, likeData, reposts, payedPosts] =
                        await Promise.all([
                              query.subscribers.findFirst({
                                    where: {
                  ...
                  const resolvedPosts = await Promise.all(
                        posts.map(async (post) => {
                              ...
                              return {
                                    ...
                                    wasReposted: postRepostSet.has(post.id),
                                    isSubscribed:
                                          checkIfUserCanViewPaidPosts || !!subs,
                              };
                        }),
                  );
                  return {
      ...
      static async UpdatePost({
            ...
            try {
                  ...
                  const hasExistingMedia =
                        existingPost.UserMedia &&
                        existingPost.UserMedia.length > 0;
                  ...
                  const updatedPost = await query.$transaction(async (tx) => {
                        ...
                        const post = await tx.post.update({
                              ...
                              data: {
                                    content: formattedContent,
                                    post_audience:
                                          (visibility as PostAudience) ||
                                          existingPost.post_audience,
                                    post_status: hasOnlyImages
                                          ? "approved"
                                          : "pending",
                                    post_price:
                                          visibility === "price"
                        ...
                        if (
                              visibility &&
                              visibility !== existingPost.post_audience
                        ) {
            ...
            try {
                  ...
                  const [updatePost, updateMedia] = await query.$transaction([
                        ...
                        query.userMedia.updateMany({
                              ...
                              data: {
                                    accessible_to: String(visibility)
                                          .trim()
                                          .toLowerCase(),
                              },
                        }),
                  ...
                  if (!updatePost || !updateMedia) {
                        return {
                              error: true,
                              message: "Could not update post audience",
                        };
                  }
      ...
      static async GetCommentReplies({
            ...
            const checkedReplies = replies.map((reply) => ({
                  ...reply.toObject(),
                  likedByme: replyLikes.some(
                        (like) => like.commentId === reply.comment_id,
                  ),
            }));
      ...
      static async LikePost({
            ...
            try {
                  ...
                  const result = await RedisPostService.likePost(
                        postId,
                        userId,
                  );
                  ...
                  if (result.isLiked && post.user_id !== userId) {
                        try {
                              ...
                              const likerHandle =
                                    liker?.username ||
                                    FormatName(liker?.name) ||
                                    "someone";
                              const likerUrl = liker?.username
                                    ? `/${liker.username}`
                                    : "/";
                              const ownerGreeting =
      ...
      static async DeletePost({
            ...
            try {
                  ...
                  await query.$transaction(async (tx) => {
                        ...
                        await tx.userRepost.deleteMany({
                              where: { post_id: post.id },
                        });
                        await tx.postGift.deleteMany({
                              where: { post_id: post.id },
                        });
      ...
      static async GiftPoints(
            ...
            try {
                  ...
                  try {
                        ...
                        console.error(
                              "Error processing gift transaction:",
                              error,
                        );
                        throw error;
                  }
      ...
      static async PayForPost(
            ...
            try {
                  ...
                  const [alreadyPurchased, post, userPoints] =
                        await Promise.all([
                              query.purchasedPosts.findFirst({
                                    where: {
                                          user_id: user.id,
                  ...
                  if (!postPrice || postPrice <= 0) {
                        return this.errorResponse(
                              "Post is not paid or is free",
                        );
                  }

                  if (!userPoints?.points || userPoints.points < postPrice) {
                        return this.errorResponse(
                              "Insufficient points to purchase this post",
                        );
                  }
      ...
      static async GetMentions({
            ...
            try {
                  ...
                  throw new Error(
                        "An unexpected error occurred while fetching mentions",
                  );
            }
      }
