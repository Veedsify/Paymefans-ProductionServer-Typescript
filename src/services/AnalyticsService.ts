...
class AnalyticsService {
  ...
  private static getCacheKey(
    ...
    return timeRange
      ? `analytics:${prefix}:${userId}:${timeRange}`
      : `analytics:${prefix}:${userId}`;
  }
  ...
  private static getDateRange(timeRange: TimeRangeKey): {
    startDate: Date;
    endDate: Date;
  } {
    const now = new Date();
  ...
  static async GetAccountGrowthData(
    userId: number,
    timeRange: TimeRangeKey,
    ...
    return this.getCachedOrExecute(
      ...
      async () => {
        ...
        if (followData.length > 0) {
          return this.processFollowerGrowthData(
            followData,
            timeRange,
            currentFollowers,
          );
  ...
  private static processFollowerGrowthData(
    followData: any[],
    timeRange: TimeRangeKey,
  ...
  private static generateSimulatedGrowthData(
    timeRange: TimeRangeKey,
    currentFollowers: number,
  ): AccountGrowthData[] {
    const data: AccountGrowthData[] = [];
      ...
      case "24hrs":
        for (let i = 0; i < 6; i++) {
          ...
          const followers = Math.max(
            0,
            currentFollowers - Math.floor(Math.random() * 50) + i * 10,
          );
          data.push({
      ...
      case "alltime":
        ...
        for (let i = 0; i < 12; i++) {
          data.push({
            ...
            followers: Math.max(
              0,
              Math.floor((currentFollowers * (i + 1)) / 12),
            ),
          });
        }
        break;
  ...
  private static groupByDay(followData: any[]): number[] {
    ...
    followData.forEach((follow) => {
      ...
      const daysDiff = Math.floor(
        (now.getTime() - followDate.getTime()) / (1000 * 60 * 60 * 24),
      );
      ...
      switch (timeRange) {
        case "24hrs":
          for (let i = 0; i < 6; i++) {
            ...
            data.push({
              name: `${hour.toString().padStart(2, "0")}:00`,
              likes: periodPosts.reduce(
                (sum, post) => sum + post.post_likes,
                0,
              ),
              comments: periodPosts.reduce(
                (sum, post) => sum + post.post_comments,
                0,
              ),
              views:
                periodViews ||
                periodPosts.reduce(
                  (sum, post) => sum + post.post_impressions,
                  0,
                ),
            });
          }
        ...
        case "7days":
          ...
          for (let i = 0; i < 7; i++) {
            ...
            const dayPosts = posts.filter(
              (post) => post.created_at >= dayStart && post.created_at < dayEnd,
            );

            const dayViews = dayPosts.reduce((sum, post) => {
              const viewsInDay = post.PostImpression.filter(
                (view) =>
            ...
            data.push({
              ...
              views:
                dayViews ||
                dayPosts.reduce((sum, post) => sum + post.post_impressions, 0),
            });
          }
          break;
        default:
  ...
  private static getSampleEngagementData(
    timeRange: TimeRangeKey,
  ): EngagementData[] {
    const data: EngagementData[] = [];
  ...
  static async GetRecentPostsData(
    userId: number,
    timeRange: TimeRangeKey,
    ...
    return this.getCachedOrExecute(cacheKey, this.CACHE_TTL.SHORT, async () => {
      ...
      return posts.map((post) => {
        ...
        const engagementRate =
          realViews > 0 ? (totalEngagement / realViews) * 100 : 0;
  ...
  static async GetMetricsData(
    ...
    return this.getCachedOrExecute(cacheKey, this.CACHE_TTL.SHORT, async () => {
      ...
      const totalRealViews = posts.reduce(
        (sum, post) => sum + post.PostImpression.length,
        0,
      );
      const totalViews =
        totalRealViews ||
        posts.reduce((sum, post) => sum + post.post_impressions, 0);
      ...
      const followerTrend = this.calculateTrend(
        user?.total_followers || 0,
        previousMetrics.followers,
      );
      const viewsTrend = this.calculateTrend(
