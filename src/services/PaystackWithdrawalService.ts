...
export class PaystackWithdrawalService {
  ...
  constructor() {
    ...
    this.baseUrl =
      process.env.PAYSTACK_PAYMENT_URL || "https://api.paystack.co";
...
export class WithdrawalRequestService {
  ...
  async getWithdrawalRequest(
    ...
    try {
      const withdrawal = await query.withdrawalRequest.findUnique({
        where: { id },
        include: {
          user: true,
          bank: true,
  ...
  async updateWithdrawalRequest(
    ...
    try {
      const withdrawal = await query.withdrawalRequest.update({
        where: { id },
        data,
        include: {
          user: true,
  ...
  async processWithdrawal(id: number): Promise<{
    ...
    try {
      ...
      const amountAfterFee = WithdrawalFeeCalculator.calculateAmountAfterFee(
        withdrawal.amount,
      );
      ...
      if (transferResult.success) {
        ...
        return {
          success: false,
          message: transferResult.message || "Failed to initiate transfer",
        };
      }
    } catch (error) {
  ...
  async finalizeWithdrawal(
    ...
    try {
      ...
      if (finalizeResult.success) {
        ...
        return {
          success: false,
          message: finalizeResult.message || "Failed to finalize transfer",
        };
      }
    } catch (error) {
