...
export default class ConversationService {
        ...
        try {
            ...
            const conversationsResult = conversations
                .map((convo) => {
                    const lastMessage =
                        messages.find(
                            (m) => m.conversationsId === convo.conversation_id,
                        ) || null;
    ...
    static async GetFreeMessageStatus({
        ...
        try {
            ...
            const freeMessageSettings =
                await query.conversationFreeMessage.findMany({
                    where: {
                        conversation_id: conversationId,
                        user: {
                            user_id: {
            ...
            const bothEnabled =
                (user1Setting?.enabled || false) &&
                (user2Setting?.enabled || false);
