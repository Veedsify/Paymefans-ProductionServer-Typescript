...
const MessagePointsWorker = new Worker(
    "messagePointsQueue",
    async (job) => {
        const { sender_id, receiver_id, messageId } = job.data as MessagePointsJobData;

        try {
            console.log("💰 Processing message points deduction:", {
                sender_id,
                receiver_id,
                messageId,
            });
            ...
            if (result.success) {
                console.log("✅ Message points processed successfully:", messageId);
            } else {
                console.error("❌ Failed to process message points:", result.message);
...
async function removePointsFromUser(
    ...
    try {
        ...
        const senderEnablesFreeMessages = sender?.Settings?.enable_free_message || false;
        const receiverEnablesFreeMessages = receiver?.Settings?.enable_free_message || false;

        // If both users enable free messages, skip point deduction
        if (senderEnablesFreeMessages && receiverEnablesFreeMessages) {
            console.log("🆓 Free message enabled by both users - skipping point deduction");
            return { success: true };
        }
...
MessagePointsWorker.on("failed", (job, err) => {
    console.error(`MessagePoints job ${job?.id} failed with error: ${err.message}`);
});
