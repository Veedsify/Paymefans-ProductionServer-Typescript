...
export default class PointController {
      ...
      static async BuyPoints(req: Request, res: Response): Promise<any> {
            try {
                  const Points = await PointService.BuyPoints(req?.user!, req.body.points_buy_id)
                  return res.status(200).json({ ...Points })
            } catch (error: any) {
                  return res.status(500).json({ error: error.message })
            }
      ...
      static async PurchasePoints(req: Request, res: Response): Promise<any> {
            try {
                  const Points = await PointService.PurchasePoints(req?.user!, req.body.amount)
                  return res.status(200).json({ ...Points })
            } catch (error: any) {
                  return res.status(500).json({ error: error.message })
            }
      ...
      static async PaymentCallBack(req: Request, res: Response): Promise<any> {
            try {
                  const Points = await PointService.PaystackPaymentCallBack({ reference: req.query.reference as string })
                  if (Points.status) {
