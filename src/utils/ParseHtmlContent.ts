import { MentionUser } from "types/post";

export default function ParseContentToHtml(
  text: string,
  mentions: MentionUser[],
): string {
  if (!text) return "";

  const location = process.env.APP_URL || "http://localhost:3000";
  ...
  mentions.forEach((m) => {
    const mentionRegex = new RegExp(
      `${String(m.username).trim()}(?![A-Za-z0-9_-])`,
      "g",
    );
