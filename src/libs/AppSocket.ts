...
async function AppSocket(io: any) {
    ...
    io.use((socket: any, next: (err?: Error) => void) => {
        ...
        if (!username || typeof username !== "string" || !userid) {
            console.error(
                "❌ Missing or invalid username or userid:",
                socket.id,
            );
    ...
    io.on("connection", (socket: any) => {
        ...
        const userid = socket.userid; // ← already validated and attached
        ...
        socket.on("join", (data: string) => {
            AddToUserRoom(data);
            SocketService.HandleJoinRoom(() => {}, socket, data);
        });
        ...
        socket.on("new-message", async (data: any) => {
            const userRoom =
                data.conversationId ||
                (await SocketService.getUserRoom(socket.id));
            const user = await SocketService.getUserFromSocket(socket.id);
            if (user && userRoom) {
        ...
        socket.on(
            ...
            async (data: { groupId: string; userId: string }) => {
                const user = await SocketService.getUserFromSocket(socket.id);
                if (user) {
                    GroupChatHandler.handleLeaveGroupRoom(
                        socket,
                        user,
                        data,
                        io,
        ...
        socket.on("send-group-message", async (data: any) => {
            ...
            if (!user?.userId) {
                ...
                socket.emit("group-error", {
                    message: "User not properly connected",
                });
                return;
            }
        ...
        socket.on("leave-all-groups", async (data: { userId: string }) => {
            ...
            try {
                ...
                const userGroupKeys = await redis.keys(
                    `group-rooms:${data.userId}`,
                );
                let leftGroups = 0;
                ...
                for (const key of userGroupKeys) {
                    ...
                    for (const [groupId, roomName] of Object.entries(
                        groupRooms,
                    )) {
                        try {
        ...
        socket.on("disconnect", async (reason: any) => {
            ...
            if (user?.userId) {
                try {
                    ...
                    console.error(
                        "Error cleaning up group rooms on disconnect:",
                        error,
                    );
                }
            }
            ...
            try {
                ...
                console.error(
                    "Error handling user inactive on disconnect:",
                    error,
                );
            }
        });
            ...
            for (const [userId, memberDataStr] of Object.entries(members)) {
                try {
                    ...
                    if (timeDiff > 60 * 60 * 1000) {
                        ...
                        const userGroupRooms = await redis.hgetall(
                            `group-rooms:${userId}`,
                        );
